...
namespace :import do
	...
	task posts: :environment do
		...
		file.each do |row|
			...
			post = Post.new(title:title,body:body,created_at:created_at,updated_at:updated_at,open_range:open_range,like_count:like_count,posted_at:posted_at)
			#post.id=post_id
			post.save
	...
	task codis: :environment do
		...
		file.each do |row|
			...
			codi = Codi.new(event:e,preference:p,created_at:c,updated_at:u)
			#codi.id=codi_id
			codi.save
	...
	task clothes: :environment do
		...
		file.each do |row|
			...
			cloth = Cloth.new(main_category:m,sub_category:s,color:co,created_at:c,updated_at:u)
			#cloth.id=cloth_id
			cloth.save
